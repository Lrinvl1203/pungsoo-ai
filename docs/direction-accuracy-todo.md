...
우선순위: 지도 기반 추정 방위는 구현 완료. 센서·도면·외관 사진 교차
검증을 포함한 현장급 정밀화는 운영 배포와 E2E 이후 `DEFERRED`.
...
- 사용자가 정적 위성지도에서 대상 위치를 확인하고 핀을 미세 조정한다.
- 선택적으로 대문·현관 외향 화살표를 지정한다. 이 값은 진북 기준
  `map_arrow`, 신뢰도 `low`의 추정치로 저장한다.
- 서버는 방위각을 결정론적인 24산 좌향으로 분류해 규칙·근거와 함께
  프롬프트에 주입한다.
- 주변 약 800m의 도로, 녹지, 수변과 지형의 시각적 특징은 분석할 수 있다.
- 사용자가 화살표를 지정하지 않으면 대문·현관 방향은 알 수 없으며
  결과를 `초견 분석`으로 표시하고 좌향을 단정하지 않는다.
- 필지 경계, 건물 외곽선, 실제 고도·경사·수계 데이터는 사용하지 않는다.
- 지도 화살표도 현장 실측이나 건축 도면이 아니므로 정밀 분석 등급으로
  승격하지 않는다.
...
- [x] 주소 검색과 정적 위성지도 핀 확인·미세 조정
- [x] 지도 위 대문·현관 외향 화살표 편집 UI
- [x] 진북 기준 방위각의 결정론적 24산 좌향 계산과 경계값 테스트
- [x] `map_arrow` 측정 방식·방위각·`low` 신뢰도 저장
- [x] 지도 추정 방위와 24산 판정을 분석 프롬프트에 사전 계산값으로 전달
- [x] 결과와 PDF에 추정 방위·측정 방식·낮은 신뢰도 표시
- [x] 방위 없는 분석을 `초견 분석`으로 명확히 표시하고 방위 단정 금지
- [ ] 해외 주소 검색 범위와 지도 타일 가용성 검증
- [ ] 외관·현관 사진 입력과 EXIF 보조자료 추출
...
- [ ] 방위 신뢰도 계산 및 재측정 안내
- [ ] 교차 검증된 확정 방위와 오차를 분석 프롬프트에 전달
- [ ] 보고서에 교차 측정 횟수·오차·중간/높음 신뢰도 표시
