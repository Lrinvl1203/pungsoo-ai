...
첫 실제 사업장 파일럿의 토·목·개 수호상 아트월 v1 3안, 영물형 v2 9안과
오브제 3안, 공통 재생성 메타 프롬프트와 제조 제약은
`pilots/PILOT-BIZ-001-CONCEPTS.md`에 보존한다. 이 파일럿 프롬프트는
해당 Site Remedy Identity의 제품 변형 기록이며 전체 런타임 정본을
대체하지 않는다.

2026-08-11 사용자 검토에서 사실적인 동물 초상은 비방화 후보에서 제외됐다.
같은 문서의 `SPIRIT ABSTRACTION LOCK`은 동물 가시성을 귀·주둥이·가슴선·꼬리
같은 종별 단서로 확보하되, 몸의 나머지는 선택 화풍의 안개·먹·광물·식물선·
기하 구조로 변환한다. 화풍은 색만 바꾸는 옵션이 아니라 동물 해부, 표면,
공간과 분위기 전체를 바꾸는 생성 규칙이다.

v2의 문화권 확장은 일본 민화·에도 목판과 유럽 중세 태피스트리·상징주의를
포함한다. 문화권 화풍은 매체·구도·분위기만 번역하고 한국 풍수에서 정한
오행·에너지 모드·개 수호상·보호 고리는 변경하지 않는다.

우키요에 내부에서도 산수 대경관, 아이즈리 야행, 고급 스리모노를 별도
라우팅한다. 여우·다중 꼬리·코마이누·시사로 종이 변형되지 않도록 짧고 넓은
주둥이, 견고한 가슴과 한 개의 꼬리를 잠그며, 텍스트·화가 서명·낙관은
생성하지 않는다.
