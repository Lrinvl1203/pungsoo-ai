...
## 2. 결제사 가격 — 2026-07-31 결정 완료

결제사 상품 가격을 올리지 않기로 했으므로, 서버 정가를 결제사 설정에 맞춰
9,900원(VAT 포함 단일가)으로 내렸다.
...
서버는 VAT를 따로 더하지 않는다. 근거는 `DECISION_LOG.md` D-028.
가격을 바꿀 때는 `services/pricing.ts`와 결제사 대시보드를 함께 바꾼다.
...
현재 report·remedy·zodiac이 각각 9,900원이다. 3개 다 사면 29,700원인데,
...
| `DECISION_LOG.md` | 확정된 결정(D-001~D-028) |
