...
Production, Preview 모두 설정한다. 설정 후 재배포해야 반영된다.
...
## 4. 결제사 상품 가격 정합 (사람이 결정해야 함)

서버가 이제 주문 종류별 정가를 검증한다(`server/pricing.ts`).
...
현재 문서상 Polar·Latpeed 상품은 9,900원으로 설정되어 있다. 이 상태로 결제하면
금액 불일치로 **확인이 거부되거나 `PENDING`으로 기록된다.**

둘 중 하나를 선택해 반영한다.

- 결제사 상품 가격을 10,890원으로 올린다(서버 정가 유지).
- 또는 `services/pricing.ts`의 정가를 9,900원으로 내리고 VAT 표기를 정리한다(코드 변경 필요).

이는 사업·세무 판단이므로 코드에서 임의로 바꾸지 않았다.
